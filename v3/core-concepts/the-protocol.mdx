...
Standard `3xx` redirects never reach these rules. The browser follows them transparently, replaying the request headers against the new location, so `X-Inertia` still reaches the redirect target and the client only ever inspects the final response in the chain, which is an ordinary Inertia response. This is why the server converts a `302` to a `303` after non-GET requests, and why external redirects, where those headers cannot survive, must instead be signaled with a `409 Conflict`.

A valid Inertia response with a status of `400` or greater fires that same cancelable HTTP exception event before rendering. Cancelling it there suppresses the render, letting you handle the failure yourself.
...
Inertia shares data between the server and client via a page object. This object includes the necessary information required to render the page component, update the browser's history state, and track the site's asset version. The page object may include the following properties:
...
The `X-Inertia-Partial-Component` header includes the name of the component that is being partially reloaded. This is necessary, since partial reloads only work for requests made to the same page component. Should the final destination differ for some reason (e.g. the user was logged out and is now on the login page), no partial reloading occurs.
...
External redirects point outside your Inertia app, so an XHR visit may not follow them. The server returns a `409 Conflict` with an `X-Inertia-Location` header, and the client performs a full `window.location` visit.
...
Each key may map to either a single message or an array of messages, and the client renders whichever shape it receives. The choice is left to the server, so the Laravel adapter sends the first error per field by default and may be [configured](/v3/the-basics/validation#multiple-errors-per-field) to send them all.
