...
Returning `null` on signed-out requests overwrites the cached user on every response, while returning a once prop when authenticated caches the user across visits.
