...
<Note>The error modal injects an inline `<style>` tag. For applications enforcing a [Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) that restricts inline styles, you may configure a [nonce](/v3/installation/client-side-setup#content-security-policy) on `createInertiaApp()`.</Note>
